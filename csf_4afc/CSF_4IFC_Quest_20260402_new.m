...
Lim_ECC = 20*Parameters.pixperdeg;
Gabor.x_ycoords(:,Gabor.ECC>Lim_ECC) = [];
Gabor.Polar(Gabor.ECC>Lim_ECC) = [];
Gabor.ECC(Gabor.ECC>Lim_ECC) = [];
    ...
    for iLoc = 1:size(Gabor.x_ycoords,2)
        ...
        if ResponseProp(iLoc) >= pThreshold*100
            ...
            if ~isempty(idxRunToKeep)
                ...
                % no previous run has been found (e.g., run 1, or keep on responding wrongfully at this location
                thresholdsPlot(iLoc) = tGuess; % Set it to starting guess
                slopesPlot(iLoc) = beta; % Set it to starting guess
            end
        end
